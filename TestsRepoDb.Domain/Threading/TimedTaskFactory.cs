namespace Mitrol.Framework.Domain.Threading
{
    ...
    public static class TimedTaskFactory
    {
        /// <summary>
                                 ...
                                 T defaultValue = default)
        {
            ...
            T mainAction()
            {
                return MainPeriodicTaskAction(intervalInMilliseconds
                    , delayInMilliseconds, duration
                    , maxIterations, cancelToken
                    , stopWatch, synchronous
                    , wrapperAction, breakCondition
        ...
        private static T MainPeriodicTaskAction<T>(int intervalInMilliseconds,
                                                    ...
                                                    T defaultValue = default)
        {
            ...
            using (ManualResetEventSlim periodResetEvent = new ManualResetEventSlim(false))
            {
                ...
                while (true)
                {
                    ...
                    subTask = Task.Factory.StartNew(wrapperAction, cancelToken, subTaskCreationOptions, TaskScheduler.Current);
                    ...
                    if (synchronous)
                    {
                        ...
                        try
                        {
                            ...
                            if (breakCondition(subTask.Result))
                                break;
                        }
                    ...
                    catch (OperationCanceledException) { /* do not let OperationCanceledException exceptions to propagate...*/ }
                    finally
                    {
                ...
                return subTask != null && subTask.IsCompleted ? subTask.Result : defaultValue;
            }
        }
